...
class DraggableScrollableController extends ChangeNotifier {
  /// Creates a controller for [CustomDraggableScrollableSheet].
  ...
  ///
  /// The sheet will not snap after calling [animateTo] even if [CustomDraggableScrollableSheet.snap]
  ...
  /// canceled as well.
  ///
  /// The sheet will not snap after calling [jumpTo] even if [CustomDraggableScrollableSheet.snap]
  /// is true. Snapping only occurs after user drags.
  ...
  void _attach(
      _CustomDraggableScrollableSheetScrollController scrollController) {
...
/// Manages state between [_CustomDraggableScrollableSheetState],
/// [_CustomDraggableScrollableSheetScrollController], and
...
class _DraggableSheetExtent {
  ...
  double get callMinSize => minSize;
  double get callMaxSize => maxSize;
...
class _CustomDraggableScrollableSheetState
  ...
  void initState() {
    ...
    _scrollController =
        _CustomDraggableScrollableSheetScrollController(extent: _extent);
    ...
    if (widget.snap &&
      ...
      WidgetsBinding.instance.addPostFrameCallback((Duration timeStamp) {
        for (int index = 0;
            ...
            index++) {
          final _CustomDraggableScrollableSheetScrollPosition position =
              _scrollController.positions.elementAt(index)
                  as _CustomDraggableScrollableSheetScrollPosition;
          position.goBallistic(0);
        }
      }, debugLabel: 'CustomDraggableScrollableSheet.snap');
    }
  }
...
/// See also:
///
///  * [_CustomDraggableScrollableSheetScrollPosition], which manages the positioning logic for
///    this controller.
///  * [PrimaryScrollController], which can be used to establish a
///    [_CustomDraggableScrollableSheetScrollController] as the primary controller for
///    descendants.
class _CustomDraggableScrollableSheetScrollController extends ScrollController {
  ...
  _CustomDraggableScrollableSheetScrollPosition createScrollPosition(
    ...
    return _CustomDraggableScrollableSheetScrollPosition(
...
/// See also:
///
///  * [_CustomDraggableScrollableSheetScrollController], which uses this as its [ScrollPosition].
class _CustomDraggableScrollableSheetScrollPosition
  ...
  void applyUserOffset(double delta) {
    ...
    // 手勢與列表由上往下 >> +delta
    // pixels >> 列表位置
    // listShouldScroll => pixels > 0.0;
    // !listShouldScroll 在列表是Top位置為true
    // !(extent.isAtMin || extent.isAtMax) Sheet在非邊界為true
    // extent.isAtMin && delta < 0 Sheet在最小高度及當列表由下往上
    // extent.isAtMax && delta > 0 Sheet在最大高度及當列表由上往下
    if ((!listShouldScroll && delta > 0) &&
    // if (!listShouldScroll &&
  ...
  void goBallistic(double velocity) {
    ...
    final AnimationController ballisticController =
        AnimationController.unbounded(
      debugLabel:
          objectRuntimeType(this, '_CustomDraggableScrollableSheetPosition'),
      vsync: context.vsync,
    );
